...
export function createPlanRenderer(){
 ...
 const order=o=>o.geometry.type==='TubeGeometry'?100:new THREE.Vector3().setFromMatrixPosition(o.matrixWorld).y;meshes.sort((a,b)=>order(a)-order(b));
 ...
 if(geo.type==='TubeGeometry'){drawPath(geo.parameters.path.getPoints(48).map(p=>project(p.applyMatrix4(o.matrixWorld),camera)),null,color,m.opacity,1.6);continue;}
