...
export function createPlanRenderer(){
 ...
 const drawPath=(pts,fill,stroke,alpha=1)=>{if(pts.length<2)return;ctx.globalAlpha=alpha;ctx.beginPath();pts.forEach((p,i)=>i?ctx.lineTo(...p):ctx.moveTo(...p));if(fill){ctx.closePath();ctx.fillStyle=fill;ctx.fill();}if(stroke){ctx.strokeStyle=stroke;ctx.lineWidth=.8;ctx.stroke();}ctx.globalAlpha=1;};
 ...
 meshes.sort((a,b)=>new THREE.Vector3().setFromMatrixPosition(a.matrixWorld).y-new THREE.Vector3().setFromMatrixPosition(b.matrixWorld).y);
 ...
 if(geo.type==='TubeGeometry'){drawPath(geo.parameters.path.getPoints(48).map(p=>project(p.applyMatrix4(o.matrixWorld),camera)),null,color,m.opacity);continue;}
